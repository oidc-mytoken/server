...
// Subtokens returns the subtoken tree.
func (s *service) Subtokens(
	...
	if err := db.Transact(
		rlog, func(tx *sqlx.Tx) error {
			...
			var rollback bool
			res, rollback = s.subtokensLogic(rlog, tx, req, mt, clientMetaData)
			if rollback {
				return errors.New("rollback")
			}
			return nil
...
func (s *service) subtokensLogic(
	...
	tokenTree, tokenUpdate, err := s.doTokenInfoSubtokens(rlog, tx, req, mt, clientMetaData, usedRestriction)
	if err != nil {
		rlog.Errorf("%s", errorfmt.Full(err))
...
func (s *service) doTokenInfoSubtokens(
	rlog log.Ext1FieldLogger, tx *sqlx.Tx, req *pkg.TokenInfoRequest, mt *mytoken.Mytoken,
	clientMetaData *api.ClientMetaData,
